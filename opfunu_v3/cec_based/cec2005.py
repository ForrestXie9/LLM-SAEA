...
class F22005(CecBenchmark):
    ...
    def evaluate(self, x, *args):
        ...
        self.check_solution(x, self.dim_max, self.dim_supported)
        ndim = len(x)
        x = x - self.f_shift[:ndim]
        f = 0
        for i in range(ndim):
            f += np.sum(x[:i+1]) ** 2  # 对每一列进行累加并求平方，然后求和
...
class F52005(CecBenchmark):
    ...
    def evaluate(self, x, *args):
        ...
        ndim = len(x)
        o = self.f_shift
        o[:int(np.ceil(ndim / 4))] = -100
        o[max(int(np.floor(0.75 * ndim)-1), 1):] = 100
        B = np.dot(self.f_matrix, o)

        f = np.max(np.abs(np.dot(self.f_matrix, x) - B))
...
class F72005(CecBenchmark):
    ...
    def evaluate(self, x, *args):
        ...
        f = 1
        for i in range(ndim):
            f = f * np.cos(z[i] / np.sqrt(i + 1))

        f = np.sum(z ** 2) / 4000 - f + 1
...
class F82005(CecBenchmark):
    ...
    def __init__(self, ndim=None, bounds=None, f_shift="data_ackley", f_matrix="ackley_M_D", f_bias=-140.):
        ...
        # self.f_shift[2 * np.arange(1, self.ndim // 2 + 1) - 1] = -32
        # self.f_shift[2 * np.arange(1, self.ndim // 2 + 1) - 1] = -32
    def evaluate(self, x, *args):
        self.n_fe += 1
        self.check_solution(x, self.dim_max, self.dim_supported)
        ndim = len(x)
        x = np.dot((x - self.f_shift), self.f_matrix)
        f = np.sum(x ** 2)
        f = 20 - 20 * np.exp(-0.2 * np.sqrt(f / ndim)) - np.exp(np.sum(np.cos(2 * np.pi * x)) / ndim) + np.exp(1)
