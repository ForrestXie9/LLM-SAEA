...
import numpy as np
from opfunu_v3.cec_based.cec import CecBenchmark
from opfunu_v3.utils import operator
...
class F112015(F102015):
    ...
    def evaluate(self, x, *args):
        ...
        mz = np.dot(self.f_matrix, x - self.f_shift)
        # print('corret_F11')
        return self.g1(600.0/100.0*mz[self.idx1]) + self.g2(0.5/100.0*mz[self.idx2]) + self.g3(2.048/100.0*mz[self.idx3]) + self.g4(mz[self.idx4]) + self.f_bias
...
class F122015(F102015):
    ...
    def __init__(self, ndim=None, bounds=None, f_shift="shift_data_12_D", f_matrix="M_12_D", f_shuffle="shuffle_data_12_D", f_bias=1200.):
        super().__init__(ndim, bounds, f_shift, f_matrix, f_shuffle, f_bias)
        self.n_funcs = 5
        ...
        # print(self.idx1,'|', self.idx2, '|',self.idx3,  '|',self.idx4, '|', self.idx5)
    ...
    def evaluate(self, x, *args):
        self.n_fe += 1
        self.check_solution(x, self.dim_max, self.dim_supported)
        shift_x = x - self.f_shift
        mz = np.dot(self.f_matrix, shift_x)
        # print('corret_F12')
        return self.g1(5.0/100*mz[self.idx1]) + self.g2(5.0/100.0*mz[self.idx2]) + self.g3(5.0/100.0*mz[self.idx3]) + \
               self.g4(1000.0/100.0*mz[self.idx4]) + self.g5(mz[self.idx5]) + self.f_bias
...
class F132015(CecBenchmark):
    ...
    def evaluate(self, x, *args):
        ...
        g0 =self.lamdas[0] * self.g0(2.048/100.0*z0) + self.bias[0]
        w0 = operator.calculate_weight(x - self.f_shift[0], self.xichmas[0])

        # 2. High Conditioned Elliptic Function f13
        z1 = np.dot(self.f_matrix[1*self.ndim:2*self.ndim, :], x - self.f_shift[1])
...
class F142015(F132015):
    ...
    def evaluate(self, x, *args):
        ...
        z0 = np.dot(self.f_matrix[:self.ndim, :], x - self.f_shift[0])
        g0 = self.lamdas[0] * self.g0(1000.0/100.0*z0) + self.bias[0]
        w0 = operator.calculate_weight(x - self.f_shift[0], self.xichmas[0])

        # 2. Rotated Rastrigin’s Function f12
        z1 = np.dot(self.f_matrix[self.ndim:2*self.ndim, :], x - self.f_shift[1])
        g1 = self.lamdas[1] * self.g1(5.12/100.0*z1) + self.bias[1]
        w1 = operator.calculate_weight(x - self.f_shift[1], self.xichmas[1])
...
class F152015(CecBenchmark):
    ...
    def evaluate(self, x, *args):
        ...
        z0 = np.dot(self.f_matrix[:self.ndim, :], x - self.f_shift[0])
        g0 = self.lamdas[0] * self.g0(5.0/100.0*z0) + self.bias[0]
        w0 = operator.calculate_weight(x - self.f_shift[0], self.xichmas[0])
        ...
        z2 = np.dot(self.f_matrix[2*self.ndim:3*self.ndim, :], x - self.f_shift[2])
        g2 = self.lamdas[2] * self.g2(1000.0/100.0*z2) + self.bias[2]
        w2 = operator.calculate_weight(x - self.f_shift[2], self.xichmas[2])

        # 4. Rotated Weierstrass Function f3
        z3 = np.dot(self.f_matrix[3 * self.ndim:4 * self.ndim, :], x - self.f_shift[3])
        g3 = self.lamdas[3] * self.g3(0.5/100.0*z3) + self.bias[3]
        w3 = operator.calculate_weight(x - self.f_shift[3], self.xichmas[3])
