#!/usr/bin/env python
# ------------------------------------------------------------------------------------------------------%
# Created by "Thieu Nguyen" at 20:11, 20/04/2020                                                        %
#                                                                                                       %
...
from numpy import sum, dot, sin, sqrt, abs, array, cos, pi, exp, ones, max, round


class Model(Root):
    def __init__(self, f_name="Non-Continuous Rotated Hybrid Composition Function 3",
    ...
    def _main__(self, solution=None):
        ...
        if problem_size == 10 or problem_size == 30 or problem_size == 50:
            self.f_matrix = "hybrid_func3_M_D" + str(problem_size)
        else:
            print("CEC 2005 F21 function only support problem size 10, 30, 50")
            return 1
        num_funcs = 10
        C = 2000
        ...
        for i in range(0, problem_size):
            if abs(solution[i] - shift_data[0][i]) >= 0.5:
